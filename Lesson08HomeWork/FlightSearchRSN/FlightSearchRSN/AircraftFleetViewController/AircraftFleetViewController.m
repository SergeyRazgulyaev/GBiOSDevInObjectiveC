...
- (void)configureAircraftFleetViewController {
    self.title = NSLocalizedString(@"aircraftFleetViewControllerTitle", "");
}
...
- (void)configureSearchController {
    ...
    self.searchController.searchBar.placeholder = NSLocalizedString(@"aircraftFleetVCSearchControllerPlaceholder", "");
...
- (nonnull __kindof UICollectionViewCell *)collectionView:(nonnull UICollectionView *)collectionView cellForItemAtIndexPath:(nonnull NSIndexPath *)indexPath {
    ...
    if (self.searchController.isActive && [self.aircraftSearchArray count] > 0) {
        ...
        NSString *PlaceholderPart1 = [NSString stringWithFormat:NSLocalizedString(@"aircraftCapacityLabelText", "")];
        NSString *inPlaceholderAircraftCapacity =  [[self.aircraftSearchArray objectAtIndex:indexPath.row] aircraftCapacity];
        NSString *PlaceholderPart2 = [NSString stringWithFormat:@"%@", inPlaceholderAircraftCapacity];
        cell.aircraftCapacityLabel.text = [PlaceholderPart1 stringByAppendingString: PlaceholderPart2];
    } else {
        ...
        NSString *PlaceholderPart1 = [NSString stringWithFormat:NSLocalizedString(@"aircraftCapacityLabelText", "")];
        NSString *inPlaceholderAircraftCapacity =  [[self.aircraftFleetArray objectAtIndex:indexPath.row] aircraftCapacity];
        NSString *PlaceholderPart2 = [NSString stringWithFormat:@"%@", inPlaceholderAircraftCapacity];
        cell.aircraftCapacityLabel.text = [PlaceholderPart1 stringByAppendingString: PlaceholderPart2];
    }
    return cell;

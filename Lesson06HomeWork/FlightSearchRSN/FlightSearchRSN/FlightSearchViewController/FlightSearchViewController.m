...
- (void) pushShowTicketsButton: (UIButton *)sender {
    if (![self.departurePlaceTextField.text isEqual: @""] && self.searchRequest.origin == nil) {
        _searchRequest.origin = [[DataManager sharedInstance] iataForCityOrAitport: self.departurePlaceTextField.text];
    }
    if (![self.arrivalPlaceTextField.text isEqual: @""] && self.searchRequest.destination == nil) {
        _searchRequest.destination = [[DataManager sharedInstance] iataForCityOrAitport: self.arrivalPlaceTextField.text];
    }

...
- (void)createPlanes {
    for (int i = 1; i < 6; i++) {
        UIImageView *plane = [[UIImageView alloc] initWithFrame:CGRectMake(-100.0, ((float)i * 100.0) + 50.0, 312.0, 100.0)];
        plane.tag = i;
        plane.image = [UIImage imageNamed:@"Boeing737800"];
...
- (void)startAnimating:(NSInteger)planeId {
    ...
    if (plane) {
        [UIView animateWithDuration:1.0 animations:^{
            plane.frame = CGRectMake(self.bounds.size.width, plane.frame.origin.y, 312.0, 100.0);
        } completion:^(BOOL finished) {
            plane.frame = CGRectMake(-312.0, plane.frame.origin.y, 312.0, 100.0);
        }];
